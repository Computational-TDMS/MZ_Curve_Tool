...
impl PeakFitter for MultiPeakFitter {
    ...
    fn fit_peak(&self, peak: &Peak, curve: &Curve, config: &Value) -> Result<Peak, ProcessingError> {
        // 多峰拟合需要所有峰的信息，这里简化为单峰拟合
        // 在实际应用中，这个拟合器应该接收所有峰的信息
        ...
        // 简化的多峰拟合（实际应该考虑邻近峰的影响）
        self.fit_multi_peak_simple(peak, &x_data, &y_data)
    }
}
...
impl MultiPeakFitter {
    ...
    fn calculate_fit_window(&self, peak: &Peak, curve: &Curve, min_width: f64, max_width: f64) -> f64 {
        // 多峰拟合需要更大的窗口来考虑邻近峰的影响
        let peak_height = peak.amplitude;
        let curve_std = curve.intensity_std;
        
        // 动态计算窗口大小，多峰拟合使用更大的窗口
        let estimated_width = (peak_height / curve_std).sqrt() * 1.0; // 比单峰拟合大
    ...
    /// 简化的多峰拟合实现
    fn fit_multi_peak_simple(&self, peak: &Peak, x_data: &[f64], y_data: &[f64]) -> Result<Peak, ProcessingError> {
        // 这里实现简化的多峰拟合
        // 实际应用中应该考虑邻近峰的影响，进行全局优化
        ...
        fitted_peak.fwhm = result.sigma * 2.355;
        fitted_peak.hwhm = result.sigma * 1.177;
        fitted_peak.peak_type = PeakType::Gaussian; // 多峰拟合中每个峰可以是不同类型
        ...
        // 添加多峰拟合相关信息
        fitted_peak.add_metadata("multi_peak_fitting".to_string(), serde_json::json!(true));
        fitted_peak.add_metadata("fitting_method".to_string(), serde_json::json!("multi_peak"));
        
        Ok(fitted_peak)
    }

    /// 最小二乘法高斯拟合（用于多峰拟合中的单个峰）
